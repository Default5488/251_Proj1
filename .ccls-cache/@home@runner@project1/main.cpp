...
ourvector<person> db_load(
  ...
  fstream readIn;
  int count = 0;
	// ourvector<ourvector<person> > personVec;
  ...
  while (!readIn.eof()) {     // Loops until file end
    ...
    if (count == 0) 										// Filters for first line
      person.nucleo = parseSeq(content); // Returns a vec<vec<char>> to store into person
    else {                   						// Filters for Name & integers
      person.name = parseName(content); // Catches name return
      person.nucleoC = parsePerson(content); // Filters for quantity of person type
    }
...
int main() {
  ourvector<ourvector<person>> database;
  string command, file;
  bool abort = false;
